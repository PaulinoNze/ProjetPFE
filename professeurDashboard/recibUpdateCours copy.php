...
if(move_uploaded_file($fotoTmp, $rutaBannerBD)){
    $UpdateCurso = ("UPDATE cours SET nomCours='$nomCours', description='$description', image='$rutaBannerBD'  WHERE id='".$coursId."' ");
    $resultadoCurso = mysqli_query($conn, $UpdateCurso);
    }
closedir($dir);

}else{
$UpdateCursos    = ("UPDATE cursos SET nomCours='$nomCours', description='$description' WHERE id='".$coursId."' ");
