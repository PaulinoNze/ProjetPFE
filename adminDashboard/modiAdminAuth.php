...
if(isset($_POST['submit'])){
    ...
    if(!empty($Cpassword)){
        ...
        if(isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK){
            $imgData = addslashes(file_get_contents($_FILES['image']['tmp_name']));
            $sql = "UPDATE admin SET nom='$nom', prenom='$prenom', email='$email',telephone='$telephone', image = '$imgData' WHERE adminId = $adminId";
            try{
                mysqli_query($conn, $sql);
                header("Location: adminInfo.php");
                exit();
        ...
        $sql = "UPDATE admin SET nom='$nom', prenom='$prenom', email='$email',telephone='$telephone'WHERE adminId = $adminId";
            try{
                mysqli_query($conn, $sql);
                header("Location: adminInfo.php");
                exit();
